...
@st.cache_data(ttl=86400)
@retry_on_error(max_retries=2)
def download_image(url: str, preserve_transparency: bool = False) -> Optional[Image.Image]:
    """Download and cache image - SIMPLIFIED AND FIXED"""
    try:
        response = requests.get(url, timeout=10)
        ...
        if preserve_transparency:
            # For logos and icons, preserve transparency
            if img.mode == 'RGBA':
                return img
            ...
            # For phone images, convert to RGB (simpler)
            if img.mode == 'RGBA':
                # Create white background and paste image
                rgb_img = Image.new('RGB', img.size, (255, 255, 255))
                rgb_img.paste(img, (0, 0), img)
                return rgb_img
            elif img.mode in ('P', 'LA', 'CMYK'):
                return img.convert('RGB')
            else:
                return img
                
    except Exception as e:
        print(f"Image download error for {url}: {str(e)}")
        return None
...
def get_icon(icon_name: str, size: int = 40) -> Optional[Image.Image]:
    ...
    img = download_image(ICON_URLS[icon_name], preserve_transparency=True)
    if img:
...
def get_logo() -> Optional[Image.Image]:
    """Get Tripple K logo with transparency"""
    img = download_image(LOGO_URL, preserve_transparency=True)
    if img:
...
def get_phone_images(phone_id: str) -> List[str]:
    """Get phone images from API - FIXED"""
    try:
            ...
            if isinstance(data, dict):
                ...
                for key in ['images', 'pictures', 'photos']:
                    if key in data and isinstance(data[key], list) and data[key]:
                        return data[key][:6]  # Limit to 6 images
            
            # If it's already a list (some APIs return array directly)
            elif isinstance(data, list):
                return data[:6]
                
    except Exception as e:
        print(f"Error getting phone images: {e}")
    
    return []

def simple_background_removal(image: Image.Image) -> Image.Image:
    """Simple background removal for white backgrounds"""
    try:
...
# ==========================================
# AD GENERATOR BASE CLASS - FIXED VERSION
# ==========================================

class AdGenerator:
    """Base class for all ad generators - FIXED"""
    ...
    def draw_spec_with_icon(self, img: Image.Image, draw: ImageDraw.ImageDraw,
        ...
        if icon:
            if icon.mode == 'RGBA':
                ...
                img.paste(icon, (x, icon_y - icon_size // 2))
        ...
        # Draw wrapped text
        lines = wrap_text(text, self.body_font, max_width)
        
        for line in lines:
            draw.text((text_x, text_y), line, fill="white", font=self.body_font)
        ...
        for i, icon_name in enumerate(icons):
            ...
            if icon:
                ...
                if icon.mode == 'RGBA':
                    ...
                    img.paste(icon, (x, y_pos))

        return img

# ==========================================
# FACEBOOK AD GENERATOR - FIXED
# ==========================================

class FacebookAdGenerator(AdGenerator):
    """Facebook ad generator (1200x630) - FIXED"""
    ...
    def generate(self, phone_data: dict, ad_elements: Dict[str, str] = None, selected_image_url: str = None) -> Image.Image:
        ...
        # Create background
        img = Image.new('RGB', (self.width, self.height), BRAND_MAROON)
        ...
        # Add phone image - SIMPLIFIED (no background removal)
        if phone_img_url:
            ...
            if phone_img:
                ...
                # Simple paste without transparency issues
                img.paste(phone_img, (x, y))
...
# ==========================================
# WHATSAPP AD GENERATOR - FIXED
# ==========================================

class WhatsAppAdGenerator(AdGenerator):
    """WhatsApp ad generator (1080x1080) - FIXED"""
    ...
    def generate(self, phone_data: dict, ad_elements: Dict[str, str] = None, selected_image_url: str = None) -> Image.Image:
        ...
        if phone_img_url:
            ...
            if phone_img:
                # Resize image
                phone_img = phone_img.resize((400, 500), Image.Resampling.LANCZOS)
                img.paste(phone_img, (left_col_x, left_col_y))
        ...
        # Phone name
        phone_name = phone_data.get("name", "Unknown Phone")
        draw.text((right_col_x, right_col_y), phone_name,
                 fill=BRAND_MAROON, font=self.title_font)
        
        bbox = self.title_font.getbbox(phone_name)
        right_col_y += (bbox[3] - bbox[1]) + 30
        ...
        for icon_name, spec_text in specs:
            if spec_text != "N/A":
                ...
                if icon:
                    if icon.mode == 'RGBA':
                        img.paste(icon, (right_col_x, right_col_y - 15), icon)
                    else:
                        img.paste(icon, (right_col_x, right_col_y - 15))
                
                # Draw spec text
                draw.text((right_col_x + 50, right_col_y), spec_text,
                         fill="black", font=self.body_font)
...
# ==========================================
# INSTAGRAM AD GENERATOR - FIXED
# ==========================================

class InstagramAdGenerator(AdGenerator):
    """Instagram ad generator (1080x1350) - FIXED"""
    ...
    def generate(self, phone_data: dict, ad_elements: Dict[str, str] = None, selected_image_url: str = None) -> Image.Image:
        ...
        if phone_img_url:
            ...
            if phone_img:
                ...
                img.paste(phone_img, (x, content_y))
            ...
            for i, (icon_name, spec_text) in enumerate(featured_specs):
                ...
                if icon:
                    if icon.mode == 'RGBA':
                        ...
                        img.paste(icon, (x_pos - 30, content_y - 30))
        ...
        # Contact section
        img = self.add_contact_section(img, self.height - 200)
                            ...
                            with col_img:
                                if images:
                                    ...
                                    try:
                                        ...
                                        if phone_img:
                                            st.markdown('<div class="phone-image-container">', unsafe_allow_html=True)
                                            st.image(phone_img, use_container_width=True, caption=phone_data['name'])
                                            st.markdown('</div>', unsafe_allow_html=True)
                                    except Exception as e:
    ...
    with tabs[2]:
        ...
        if not st.session_state.current_phone:
            ...
            if selected_image_url:
                st.info(f"Using image: {selected_image_url[:50]}...")
            else:
                st.info("Using default phone image")
            ...
            if st.session_state.generated_ads:
                ...
                for ad_type, img_bytes in st.session_state.generated_ads.items():
                    with st.expander(f"{ad_type}", expanded=True):
                        ...
                        safe_name = phone_data.get("name", "phone").replace(" ", "_").replace("/", "_")
                        filename = f"tripplek_{safe_name}_{ad_type.split()[0].lower()}_ad.png"
